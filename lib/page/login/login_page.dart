...
class _LoginPageState extends State<LoginPage> {
  ...
  bool isCheck = true;
  ///原来的UserEntity不好改，解析有问题，插件直接生成的不好改，直接新建一个对象替换，少量可以这么搞，如果对象涉及复杂还是得慢慢改了再说
  UserNewEntity _userNewEntity;
  @override
  void initState() {
  ...
  _check() {
    if (_accountController.text==null||_accountController.text.length <= 0) {
      ...
      return;
    } else if (_pwdController.text==null||_pwdController.text.length <= 0) {
      Toast.show("用户密码不能为空", context);
      return;
    } else if (_accountController.text==null||_accountController.text.length != 11) {
      Toast.show("手机号格式异常", context);
      return;
    } else if (_pwdController.text==null||_pwdController.text.length > 6 ||
  ...
  _toLogin() {
    ...
    HttpUtil.instance.post(Api.LOGIN_URL, parameters: map).then((value) => {
          print("登录结果$value"),
      setState(() {
        _userNewEntity = UserNewEntity().fromJson(json.decode(value.toString()));
            print(_userNewEntity.code);
      }),
          if (_userNewEntity.code == "0")
            {
              SharedPreferenceUtils.saveShareData(Constant.userbean, value.toString()),
              SharedPreferenceUtils.saveShareData(Constant.username, _userNewEntity.data.username),
              Navigator.of(context).pop(),
              NavigatorUtil.goMainHomePage(context),
            }
          else
            {
              Toast.show(_userNewEntity.msg, context),
            }
        });
  }
